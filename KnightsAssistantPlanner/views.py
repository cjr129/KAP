...
def CalendarNp(request):
    ...
    context_dictionary = {'monthstr': month, 'yearstr': year, 'currentDay': currentDay,
                          'fstDayOfMonth': range(fstDayOfMonth + 1), 'daysInMonth': daysInMonth}
...
def Calendar(request, Date):
    ...
    context_dictionary = {'monthstr': month, 'yearstr': year, 'fstDayOfMonth': range(fstDayOfMonth + 1),
...
def myHealthNp(request):
    ...
    actionUrl = "/kap/Calendar/" + str(month) + "-" + str(year) + "/"
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            object.year = year
            # processing start
            intensity = str(request.POST("intensity"))#form.cleaned_data.get("intensity")
            large_muscle = str(request.POST("large_muscle"))
            small_muscle = str(request.POST("small_muscle"))#form.cleaned_data.get("small_muscle")
...
def workout_json_string(workout_list):
    ...
    for workout in workout_list:
        ...
        colon = "\":"
        closing_bracket = '}, '
        json_string = json_string + open_bracket + str(
            cal_count) + colon + "\"" + workout.large_muscle + colon + "\"" + workout.small_muscle + "\"" + closing_bracket
    json_string += ']}\''
    return json_string
